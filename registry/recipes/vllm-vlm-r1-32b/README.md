...
Official OpenBMB `VLM-R1-32B` baseline for operators who want a higher-capacity multimodal reasoning model behind a local OpenAI-compatible vLLM endpoint.

## What it provides

- OpenAI-compatible `/v1` API via the official `vllm/vllm-openai` container
- `openbmb/VLM-R1-32B` served as a single-model multimodal runtime
- Image-grounded chat workflows through the Chat Completions API
- Stronger fit for premium visual reasoning, OCR-heavy documents, chart reading, and screenshot analysis
...
- `vllm`: OpenAI-compatible vLLM API server hosting `openbmb/VLM-R1-32B`
...
Review `registry/recipes/vllm-vlm-r1-32b/.env` before launch and adjust as needed:
...
- The 32B checkpoint has materially higher VRAM pressure than the 7B variant, so the default memory-utilization setting is intentionally aggressive and may still require adjustment on some systems.

## Example request

```text
curl http://localhost:9001/v1/chat/completions -H "Content-Type: application/json" -d '{"model":"openbmb/VLM-R1-32B","messages":[{"role":"user","content":[{"type":"text","text":"Summarize the anomalies in this dashboard."},{"type":"image_url","image_url":{"url":"https://example.com/chart.png"}}]}]}'
```
...
- auditing the roadmap and continuing in strict order after `VLM-R1 7B`
...
- Upstream model: `openbmb/VLM-R1-32B`
